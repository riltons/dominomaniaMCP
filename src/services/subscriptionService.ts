...
export const subscriptionService = {
  ...
  startUserTrial: async (userId: string, planSlug: string): Promise<SubscriptionWithPlan> => {
    ...
    const { data, error } = await supabaseMCP
      .from('subscriptions')
      .insert({ user_id: userId, plan_id: plan.id, status: 'trialing', starts_at: startsAt, ends_at: endsAt })
      .select('*, plans(*)')
      .single();
    if (error) throw error;
    return data as SubscriptionWithPlan;
  ...
  assignFreePlan: async (userId: string): Promise<SubscriptionRow> => {
    ...
    const { data, error } = await supabaseMCP
      .from('subscriptions')
      .insert({ user_id: userId, plan_id: plan.id, status: 'active', starts_at: now, ends_at: null })
      .single();
    if (error) throw error;
    return data as SubscriptionRow;
  },
